...
class ExpModel(pl.LightningModule):
    ...
    def set_criterion(self):
        ...
        if unsupervised_training:
            ker = self.cfg.training.kernel
            if ker == "poly":
                kernel = PolynomialKernel(max_samples=4096, add_regularization=self.cfg.training.add_regularization)
            elif ker == "rbf":
                kernel = RBFKernel(max_samples=4096, add_regularization=self.cfg.training.add_regularization)
            elif ker == "gaussian":
                kernel = GaussianKernel(max_samples=4096, add_regularization=self.cfg.training.add_regularization)
            elif ker == "anovasine":
                kernel = ANOVASineKernel(max_samples=4096, add_regularization=self.cfg.training.add_regularization)
            # breakpoint()
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        self.log("epoch_duration_train", epoch_duration)
